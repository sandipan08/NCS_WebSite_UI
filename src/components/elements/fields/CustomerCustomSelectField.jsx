            ...
            >
                ...
                {options && options?.map(option => {
                    ...
                    if (option?.name?.startsWith("-")) {
                        ...
                        return <option key={option?._id} value={option?._id + "||" + option?.name} >{`${option?.companyName} - ${option?.customerId} - ${option?.city} - (${option.isExport ? option?.countryRegion.map(ele => ele.text) : option?.state}) - ${option?.name}`}</option>
                    }
                })}
